...
export default defineConfig(({ mode }) => {
  ...
  /**
   * Resolve keys prioritizing Vercel Postgres system variables
   */
  const API_KEY = env.API_KEY || process.env.API_KEY || env.VITE_API_KEY || process.env.VITE_API_KEY;
  const VITE_API_URL = env.VITE_API_URL || process.env.VITE_API_URL || env.API_URL || '';
